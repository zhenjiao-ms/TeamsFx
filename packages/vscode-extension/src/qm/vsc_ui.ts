          ...
          try {
            ...
            if(option.onDidChangeSelection){
              const changeHandler = async function(items:QuickPickItem[]):Promise<any>{
                ...
                const oldIds = toIdSet(optionItems);
                if(option.onDidChangeSelection){
                  const newIds = (await option.onDidChangeSelection(toIdSet(optionItems), prevIds));
                  if(!isSame(oldIds, newIds)){
                    quickPick.selectedItems = Array.from(newIds).map(id=>optionMap.get(id)!);
                  }
                }
              };
              disposables.push(
