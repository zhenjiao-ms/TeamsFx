...
do
  if [ "${filename##*.}"x = "nupkg"x ];then
    ...
    nuget_package_version=$(echo $filename | sed 's/TeamsDevFunction.//' | sed 's/.nupkg//')
    break
  fi
done
...
cd $test_space_main
func extensions install --package TeamsDevFunction --version $nuget_package_version --source $local_nuget_path
